...
//     } catch (e) {
//       this.isRemoteAvailable = false;
//       console.warn(`[ToeflSync] Offline â€“ ${setId} delete queued locally only:`, e.message);
//       throw e;
//     }
//     const localSets = this._safeParse(localStorage.getItem(this._setsV2LocalKey), {});
//     delete localSets[setId];
...
class ToeflStorageSync {
  constructor() {
    ...
    this._lastStorageError = "";
    this._lastUploadInfo = null;
    this._runtimeApiGatewayUrl = null;
  }
  ...
  async _resolveRuntimeApiGatewayUrl() {
    if (this._runtimeApiGatewayUrl !== null) return this._runtimeApiGatewayUrl;

    this._runtimeApiGatewayUrl = "";
    if (typeof window !== "undefined") {
      const inlineUrl = String(window.__TOEFL_API_GATEWAY_URL__ || "").trim().replace(/\/+$/, "");
      if (inlineUrl) {
        this._runtimeApiGatewayUrl = inlineUrl;
        return this._runtimeApiGatewayUrl;
      }
    }

    if (typeof fetch !== "function") return this._runtimeApiGatewayUrl;
    ...
    try {
      ...
      if (!response.ok) return this._runtimeApiGatewayUrl;

      const payload = await response.json().catch(() => ({}));
      const runtimeUrl = String(payload?.gatewayUrl || payload?.apiGatewayUrl || payload?.url || "").trim().replace(/\/+$/, "");
      if (runtimeUrl) {
        this._runtimeApiGatewayUrl = runtimeUrl;
      }
    } catch {
      // No hosted runtime config is fine; local overrides still work.
    }
  ...
  async upsertSetRecordWithType(record, testType = "mocktest") {
    ...
    try {
      ...
      this.isRemoteAvailable = false;
      console.warn(`[ToeflSync] Offline â€“ ${setId} set saved locally only:`, e.message);
    }
    const local = this._safeParse(localStorage.getItem(this._setsV2LocalKey), {});
  ...
  _getApiGatewayBaseCandidates(runtimeApiGatewayUrl = "") {
    const baseOverride = String(localStorage.getItem("toefl_api_gateway_url") || "").trim();
    if (baseOverride) {
      return [baseOverride.replace(/\/+$/, "")];
    }
  ...
  getApiGatewayDebugInfo() {
    ...
    return {
      ...
      "Runtime Gateway URL": this._runtimeApiGatewayUrl || "(not loaded)",
  ...
  async saveAudioViaGateway(setId, audioBlob, partId = 1) {
    ...
    const fileName = String(prepared.originalName || `part_${partId}.mp3`);
    const fileType = String(prepared.contentType || "audio/mpeg");
    const runtimeApiGatewayUrl = await this._resolveRuntimeApiGatewayUrl();
    const apiBases = this._getApiGatewayBaseCandidates(runtimeApiGatewayUrl);
    const errors = [];
