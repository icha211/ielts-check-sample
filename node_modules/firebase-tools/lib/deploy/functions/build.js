...
exports.applyEnvSecretBindings = applyEnvSecretBindings;
...
async function resolveBackend(opts) {
    const { paramValues: paramValues, secretRefs: secretRefs } = await params.resolveParams(opts.build.params, opts.firebaseConfig, envWithTypes(opts.build.params, opts.userEnvs), opts.nonInteractive, opts.isEmulator, opts.force);
...
function applyEnvSecretBindings(build, envSecrets) {
    if (envSecrets.empty) {
        return;
    }
